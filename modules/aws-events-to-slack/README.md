...
| <a name="requirement_http"></a> [http](#requirement\_http) | >= 3.0, < 4.0.0 |
...
| <a name="input_health_detail_types"></a> [health\_detail\_types](#input\_health\_detail\_types) | EventBridge detail-type values to match on AWS Health events. The default matches the standard health feed. Set to [] to drop the detail-type condition entirely and forward every AWS Health detail-type (e.g. "AWS Health Abuse Event"), which also matches rules created before this filter existed. | `list(string)` | <pre>[<br>  "AWS Health Event"<br>]</pre> | no |
